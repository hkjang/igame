...
`v0.7.8`은 부서·팀 랭킹이 조직 공개 정책을 우회하던 문제를 고친 패치 릴리스입니다. `/api/v1/rankings`는 부서·팀 분기를 먼저 집계해 응답한 뒤에야 개인정보 정책을 읽었습니다. 그래서 조직 공개(`show_department`)를 꺼 두면 개인 항목에서는 부서가 가려지는데도, `rankings:read` 권한만 있으면 모든 부서·팀의 이름과 인원수·합계 점수를 그대로 열거할 수 있었습니다. Defense 랭킹은 같은 요청을 이미 `403 organization_ranking_hidden`으로 막고 있었으므로 의도는 정해져 있었고 이 핸들러만 어긋나 있었습니다. 이제 정책 판정이 첫 집계보다 먼저 일어나고, 모르는 `group` 값은 질의 전에 `400 invalid_group`으로 거부하며, 정책을 읽지 못하면 "공개"로 넘기지 않고 `503 privacy_setting_unavailable`을 반환합니다. 개인 항목에서는 부서와 함께 팀도 내리는데, 팀 역시 조직명이고 랭킹 화면이 부서가 없을 때 팀을 대신 보여 주기 때문입니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠는 그대로이며, API 계약은 이 랭킹 정책 판정이 더해진 것 외에는 변경이 없습니다. `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
