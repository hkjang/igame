...
`v0.7.10`은 릴리스 워크플로의 잠긴 Node 의존성 감사에 걸려 태그가 나가지 못하던 문제를 고친 패치 릴리스입니다. Release workflow는 Web·SDK 두 트리를 `npm audit --audit-level=low`로 검사하므로, 아무에게도 배포되지 않는 개발 도구에 대한 것이라 해도 권고가 하나라도 남아 있으면 릴리스가 중단됩니다. `v0.7.9`는 정확히 이 단계에서 두 번 멈췄습니다. GHSA-82fw-gwwq-j7x9은 `2.1.0`부터 `4.1.10`까지의 모든 vitest를 덮어 잠겨 있던 `3.2.7`이 그 범위 안에 있었고, GHSA-2883-xcg3-v3hh은 `4.3.2` 미만의 js-yaml을 덮었으며, 두 트리가 같은 권고 두 개를 함께 들고 있었습니다. 일반 CI의 감사 gate는 `--omit=dev`로 돌면서 dev 권고를 job summary에만 적기 때문에 PR은 끝까지 초록이었고, 처음 빨개진 곳이 릴리스였습니다. 게이트를 느슨하게 하는 대신 의존성 쪽을 고쳤습니다. vitest는 다른 수정 라인인 `4.1.11`이 아니라 `5.0.0`으로 올립니다. 두 workflow와 builder image가 쓰는 Node 22에 딸려 오는 npm `10.9.8`이 vitest 4의 peer set을 풀다가 `Cannot read properties of null (reading 'edgesOut')`으로 죽어, 이 저장소에서도 빈 디렉터리에서도 `4.1.11`은 설치 자체가 되지 않기 때문입니다. js-yaml은 eslint가 이미 요구하는 범위 안의 `4.3.2` 패치입니다. SDK에는 포털과 같은 `vite ^7.1.3`을 함께 명시했는데, SDK 쪽에는 vitest의 vite peer를 제약하는 것이 없어 업그레이드가 한쪽 테스트 toolchain에만 vite 8과 rolldown을 끌어왔기 때문입니다. 바뀐 것은 테스트·빌드 toolchain의 lockfile뿐이며 제품 코드, 스키마·마이그레이션, 환경변수, API 계약과 게시된 콘텐츠는 모두 그대로입니다. `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.10`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
