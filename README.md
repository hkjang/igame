...
`v0.7.7`은 `/assets/`가 빌드가 만든 번들 파일 이름을 목록으로 그대로 내주던 문제를 고친 패치 릴리스입니다. 정적 핸들러는 "열리면 파일"로 판단해 경로가 열리기만 하면 곧장 파일 서버에 넘겼는데, 디렉터리도 열립니다. 그래서 `/assets/`는 번들의 모든 파일 이름이 링크로 박힌 디렉터리 목록을 200으로 돌려줬고 `/assets`는 슬래시를 붙이는 리다이렉트를 냈습니다 — SPA로 떨어졌어야 할 요청이 빌드 산출물의 구조를 그대로 보여 주고 있었습니다. 이제 정규 파일만 서빙하고 디렉터리는 기존 규칙으로 떨어지므로, 확장자가 없는 경로는 SPA index를 받고 확장자가 있는 경로는 404를 받습니다. 저장소에는 `dist/index.html` 자리표시자만 있어 테스트가 실제 번들 모양을 세울 수 있도록 `Handler()`를 `handlerFor(fs.FS)`로 분리했습니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.7`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
