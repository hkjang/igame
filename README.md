...
`v0.7.15`는 잘린 감사 로그 CSV가 완전한 것처럼 내려가던 문제를 고친 패치 릴리스입니다. `GET /api/v1/admin/audit?format=csv`는 `200`과 CSV 헤더를 먼저 쓰고 나서야 커서를 읽는데, 도중에 데이터베이스 읽기가 실패하면 서버 로그만 남기고 정상 종료해서 클라이언트는 그냥 끝나는 파일을 받았습니다 — 완전해 보이는 부분 내보내기는 내보내기가 없는 것보다 나쁘고, `audit.export` 항목도 잘린 개수를 전체인 양 기록했습니다. 이제 파일은 완전한 것으로 통하지 않습니다. 마지막 행으로 `export.truncated`(앞선 행 수와 서버 로그에서 찾을 `request_id`)를 쓰고, `audit.export` 항목에 `truncated: true`를 남기며, 마지막 chunk 없이 연결을 끊어 브라우저에는 다운로드 실패로 보이게 합니다. 부분 파일을 남기는 도구로 받았다면 마지막 줄이 표식이므로 다시 내보내면 됩니다. 행 루프는 작은 커서 인터페이스를 받는 `writeAuditCSV`로 옮겨 DB 없이 단위 테스트를 두었고, 끝에서 끝까지의 동작은 `IGAME_TEST_DSN` 테스트가 실제 라우터를 TCP로 띄워 클라이언트가 읽는 도중 내보내기 backend를 끊고 unexpected EOF·표식 행·감사 항목을 확인합니다. 정상적으로 끝난 내보내기의 파일과 감사 항목은 전과 같습니다. 관리자 가이드의 감사 로그 항목, API 문서, 운영 문서가 동작을 설명합니다. `v0.7.14`가 더한 OIDC 자동 로그인, `v0.7.13`이 더한 nonce 기반 CSP 아래의 방문 추적 스니펫, `v0.7.12`가 발행한 실제 화면이 실린 사용자·관리자 가이드, `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.15`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
