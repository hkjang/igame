...
`v0.7.16`은 점수가 같은 랭킹 행의 순서가 정해져 있지 않던 문제를 고친 패치 릴리스입니다. 세 랭킹 endpoint(`/api/v1/rankings`, Defense Series, RealmGuard) 모두 `row_number()`를 점수만으로 매기고 페이지도 점수만으로 정렬했기 때문에, 동점인 행의 순서는 PostgreSQL의 hash aggregate가 건네주는 순서 그대로였습니다. 그 순서는 표와 함께 움직입니다 — 동점과 무관한 사람이 낮은 점수를 하나 올리면 아무의 점수도 바뀌지 않았는데 동점 1위가 다른 사람으로 바뀌었습니다. 이제 catalog·Defense·RealmGuard 8개 랭킹 쿼리는 window와 페이지 정렬 양쪽에서 같은 tiebreaker로 동점을 가릅니다: 개인은 `user_id`, 부서·팀·영웅은 이름 오름차순이며 RealmGuard 개인 랭킹은 기존대로 먼저 세운 기록(`created_at`)이 앞서고 그다음 `id`입니다. `rank`는 언제나 응답 행 순서와 같고, 같은 점수의 순서는 다른 사람이 점수를 올려도 바뀌지 않습니다. 검증은 `IGAME_TEST_DSN` 테스트가 3,000명의 동점 플레이어를 심고 ANALYZE 해 프로덕션 실행 계획을 타게 한 뒤 실제 라우터로 11개 랭킹 페이지를 받아 동점 순서와, 무관한 점수가 들어온 뒤에도 페이지가 그대로인지를 확인합니다. 동점이 없는 랭킹의 응답은 전과 같고 웹 화면은 `rank` 필드를 그대로 쓰므로 바뀌지 않습니다. API 문서가 동점 순서 규칙을 설명합니다. `v0.7.15`가 고친 잘린 감사 로그 CSV의 완전한 것처럼 보이던 내보내기, `v0.7.14`가 더한 OIDC 자동 로그인, `v0.7.13`이 더한 nonce 기반 CSP 아래의 방문 추적 스니펫, `v0.7.12`가 발행한 실제 화면이 실린 사용자·관리자 가이드, `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.16`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
