...
`v0.7.17`은 게임에 묶이지 않은 포털 공통 업적을 클라이언트가 해제할 수 없던 문제를 고친 패치 릴리스입니다. 관리자 화면은 게임별 업적과 함께 게임이 비어 있는 포털 공통 업적을 만들 수 있고, 게임 클라이언트가 업적을 해제하는 유일한 경로는 `criteria.client_unlockable`을 켠 업적에 `POST /api/v1/me/achievements`를 보내는 것입니다. 그런데 이 endpoint는 업적과 세션을 `game_id`가 같다는 조건으로만 묶었기 때문에 `game_id`가 비어 있는 업적은 어떤 세션과도 맞지 않아 `client_unlockable`을 켜 두어도 언제나 `403 achievement_not_unlockable`이었습니다 — 만들 수는 있지만 쓸 수는 없는 업적이었습니다. 이제 게임에 묶인 업적은 종전대로 그 게임의 세션에서만, 포털 공통 업적은 어느 게임의 세션에서든 해제됩니다. `client_unlockable`, 본인 세션, 세션 토큰 hash, 세션 상태 조건은 그대로이고 이미 해제된 업적의 재요청은 종전처럼 `200`입니다. 검증은 `IGAME_TEST_DSN` 테스트가 실제 라우터를 띄워 세션 쿠키로 진짜 게임 세션을 만든 뒤, 포털 공통 업적이 다른 게임의 세션에서 `201`·재요청 `200`·`/api/v1/me/achievements`에 나타나는지, 게임 A의 업적이 게임 B의 세션에서는 `403`이고 A의 세션에서는 `201`인지, 서버 규칙 업적·잘못된 토큰·모르는 세션은 여전히 `403`인지를 확인합니다. 시드된 포털 공통 업적은 모두 서버 규칙(`play_count`) 업적이라 기존 데이터의 동작은 바뀌지 않습니다. API 문서가 해제 규칙을 설명합니다. `v0.7.16`이 고친 동점 랭킹 행의 정해지지 않은 순서, `v0.7.15`가 고친 잘린 감사 로그 CSV의 완전한 것처럼 보이던 내보내기, `v0.7.14`가 더한 OIDC 자동 로그인, `v0.7.13`이 더한 nonce 기반 CSP 아래의 방문 추적 스니펫, `v0.7.12`가 발행한 실제 화면이 실린 사용자·관리자 가이드, `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
