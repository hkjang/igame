...
`v0.7.9`는 관리자 비밀번호 재설정이 대상 사용자의 세션을 지우지 않던 문제를 고친 패치 릴리스입니다. 재설정은 운영자가 탈취당한 계정을 되찾는 수단인데, `updateUser`는 `password_hash`만 쓰고 `auth_sessions`는 그대로 두었습니다. 그래서 옛 비밀번호로 열린 세션이 남은 수명(최대 12시간) 동안 그대로 살아 있었고, 훔친 쿠키는 계속 통했으며 계정은 사실 회수되지 않았습니다. 본인 변경 경로인 `changePassword`는 처음부터 나머지 세션을 지우고 있었으므로 의도는 이미 정해져 있었습니다. 이제 세션 삭제가 비밀번호 갱신과 같은 statement 안에서 함께 일어나 "새 비밀번호 + 옛 세션" 상태가 생길 수 없고, 살아남는 것은 관리자가 이 화면에서 자기 비밀번호를 재설정할 때의 요청 자신의 세션 하나뿐입니다. 역할과 상태는 `authenticate`가 매 요청 다시 읽으므로 함께 쓸어낼 필요가 없습니다. 감사 항목에는 폐기한 세션 수가 `sessions_revoked`로 남습니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.9`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
