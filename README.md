...
`v0.7.1`은 관리자 콘솔과 포털에서 실제로 막혀 있던 동작을 정리한 패치 릴리스입니다. Keycloak OIDC 설정을 저장할 수 없던 문제를 고쳤습니다 — 설정 조회 응답이 저장 요청 쪽에서 거부하는 필드를 값 안에 담고 있어, 화면이 받은 그대로 돌려보내면 `json: unknown field`로 끝났습니다. 두 게임이 점수를 기록하지 못하던 문제, 전투를 시작하면 HUD가 화면 밖에 있던 문제, 밝은 화면에서 게임 글자가 배경과 같은 색으로 그려지던 문제, 아무것도 고치지 않고 저장만 해도 일정이 바뀌던 문제도 함께 고쳤습니다. 관리자 표는 스키마 어휘 대신 사람이 읽는 값을 보여주고, 한국어와 좁은 화면에서 헤더가 창 밖으로 밀려나지 않습니다. 키보드 focus 표시, 독자가 키운 글꼴 크기, 가로 오버플로, 실제 렌더링된 대비를 릴리스 게이트가 직접 측정하며, 거부된 요청과 계정·설정 변경은 무엇이 어떻게 바뀌었는지까지 감사 로그에 남습니다. telemetry 수신 시각이 트랜잭션 시작 시각으로 기록되던 문제를 고치는 마이그레이션 `009_receipt_clock.sql` 하나가 추가되며, 그 밖의 스키마와 환경변수는 그대로입니다. `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.1`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
