...
`v0.7.13`은 관리자가 화면에서 방문 추적 스크립트를 붙일 수 있게 한 패치 릴리스입니다. 포털의 콘텐츠 보안 정책은 `script-src 'self'`로 잠겨 있어 붙여 넣은 스니펫을 브라우저가 조용히 막았고, 이미지를 다시 만들지 않고는 넣을 자리도 없었습니다. 새 관리 화면 `/admin/tracking`(메뉴 "방문 추적")에서 제공자를 고르고 저장하면 됩니다. 사내 수집기 Momento가 첫 자리이고 Google Analytics 4·Google Tag Manager·Matomo·직접 붙여넣기(8KB 이하)를 지원하며, 삽입 위치와 관리 화면 포함 여부를 함께 정합니다. 추적이 켜진 페이지에서만 서비스가 요청마다 nonce를 만들어 스니펫의 모든 `<script>` 태그에 붙이고 같은 값을 `script-src 'nonce-…'`로 정책에 넣으며, 스니펫이 부르는 출처와 신고용 `report-uri`를 더합니다. `'unsafe-inline'`은 어떤 경우에도 넣지 않고, 추적을 끄면 정책은 바이트 단위로 원래대로 돌아갑니다. API·MCP·상태·프록시 경로는 `default-src 'none'`으로 좁혔습니다. Momento는 기본으로 같은 오리진 프록시(`/momento/*` → 수집기)를 쓰므로 정책에 외부 출처가 등장하지 않고, 프록시는 세션 쿠키와 Bearer 키를 떼고 넘기며 Momento+프록시 설정이 아니면 `404`만 냅니다. 브라우저가 정책에 막힌 출처를 `POST /api/v1/tracking/csp-report`로 신고하면 같은 화면의 "브라우저가 차단한 출처" 표에 쌓이고, "허용" 한 번으로 허용 출처에 더해지며 감사 로그에 `setting.update`로 남습니다. 기본값은 꺼짐이라 설정을 저장한 적 없는 배포는 정책과 셸이 전과 같습니다. 설정은 기존 `settings` 저장소의 `tracking` 키를 쓰므로 스키마·마이그레이션과 환경변수, 게시된 콘텐츠는 그대로이며 API에는 추적 endpoint 다섯 개가 더해졌습니다. 관리자 가이드 3.3 방문 추적과 7.5 보안 항목이 설정 순서와 정책이 느슨해지지 않는 이유를 설명합니다. `v0.7.12`가 발행한 실제 화면이 실린 사용자·관리자 가이드, `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.13`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
