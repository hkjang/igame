...
`v0.7.2`는 로그인 복귀와 목록 검색 두 곳의 결함을 고친 패치 릴리스입니다. SSO 로그인 복귀 주소 검사가 scheme 없는 `//host/path` 형태를 걸러내지 못해, `return_to`가 그렇게 적힌 link를 따라간 사용자가 Keycloak 인증을 정상적으로 마친 직후 외부 사이트로 넘겨졌습니다. 이제 host·scheme·userinfo가 조금이라도 섞인 복귀 주소는 모두 `/`로 되돌립니다. 관리자 사용자·감사 로그 목록, 감사 로그 CSV 내보내기와 게임 카탈로그 검색은 검색어의 `%`와 `_`를 ILIKE 와일드카드로 흘려보내, `50%`나 `user_id`처럼 그 문자가 든 검색어에 필터가 걸린 것처럼 보이면서 전체 행을 돌려주고 있었습니다. 이제 검색어를 이스케이프한 뒤 패턴을 만들며, 빈 검색어가 필터를 건너뛰던 기존 동작과 카탈로그 태그의 정확 일치는 그대로입니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.2`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
