...
`v0.7.14`는 Keycloak에 이미 로그인한 사람을 로그인 화면 없이 들여보내는 OIDC 자동 로그인을 더한 패치 릴리스입니다. 사내 SSO에 세션이 있어도 igame을 열 때마다 로그인 화면을 한 번 거쳐야 했습니다. 관리 화면 OIDC 카드의 새 "자동 로그인" 스위치(설정 `auto_login`, 기본 꺼짐)를 켜면 포털은 화면을 그리기 전에 제공자에 `prompt=none`으로 한 번 조용히 물어봅니다 — 세션이 있으면 code를 받아 평소 흐름대로 로그인이 끝나고, 없으면 제공자가 `login_required`로 돌려보내 종전과 같은 로그인 화면이 뜹니다. 핵심은 되풀이하지 않는 것입니다. 거절된 `prompt=none`을 다시 시도하면 브라우저가 제공자와 포털 사이를 끝없이 오가므로 세 겹의 규칙이 겹칩니다: sessionStorage에 남는 표시로 탭 세션마다 한 번만 시도하고, 스스로 로그아웃한 뒤에는 다시 세션이 생길 때까지 시도하지 않으며, callback이 주소에 남기는 `sso=none` 표시가 저장소를 비운 뒤에도 재시도를 막습니다. 저장소를 읽을 수 없으면 이미 시도한 것으로 칩니다. `/login`과 API·MCP·상태·프록시 경로에서는 시작하지 않고, 시도는 숨은 iframe이 아니라 최상위 이동입니다. 판단은 주소가 아니라 서버가 합니다. `GET /api/v1/auth/oidc/login?prompt=none`은 `auto_login`이 켜져 있을 때만 제공자에 `prompt=none`을 전달하고, 꺼져 있으면 조용히 평범한 로그인으로 바꿉니다. flow 행에 조용한 시도였는지를 기록하는 마이그레이션 `010_silent_sso.sql`이 함께 실리므로, callback은 조용한 flow의 거절만 `302 /login?sso=none[&return_to=…]`(이 서비스 안의 경로만)로 보내고 state는 어느 쪽이든 소비하며, 조용하지 않은 flow의 제공자 오류는 종전대로 `401`입니다. `GET /api/v1/public/config`는 OIDC가 켜져 있고 `auto_login`도 켜진 경우에만 `oidc_auto_login`을 `true`로 내립니다. 로그인 화면은 `return_to`를 읽어 깊은 링크로 돌아가고 "사내 SSO로 계속" 버튼은 켜고 끔과 무관하게 언제나 제공자 화면을 거칩니다. 기본값은 꺼짐이라 설정을 저장한 적 없는 배포는 전과 같이 동작합니다. 관리자 가이드의 OIDC 항목과 장애표, API 문서, Keycloak 안내가 설정과 루프 방지 규칙을 설명합니다. `v0.7.13`이 더한 nonce 기반 CSP 아래의 방문 추적 스니펫, `v0.7.12`가 발행한 실제 화면이 실린 사용자·관리자 가이드, `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.14`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
