...
`v0.7.12`는 사용자 가이드와 관리자 가이드를 실제 화면과 함께 발행한 문서 릴리스입니다. 저장소에는 화면이 하나도 실리지 않은 사용자 가이드 한 편만 있었고 관리자 가이드는 아예 없었습니다. `docs/USER_GUIDE.md`와 `docs/ADMIN_GUIDE.md`가 그 자리를 대신하며, 두 문서의 그림은 모두 `v0.7.11` 서비스를 실제로 띄워 1440×900 데스크톱 창에서 찍은 화면입니다. 캡처는 `scripts/capture-guide-screenshots.sh`가 만듭니다. 일회용 서비스에 지어낸 사용자·점수·공지·이벤트·시즌을 심은 뒤, 릴리스 브라우저 gate가 이미 쓰는 고정된 Playwright 이미지로 화면을 훑습니다. 대상의 데이터를 지우는 도구이므로 `IGAME_GUIDE_CAPTURE_DISPOSABLE=yes` 없이는 실행을 거부하고, 다른 스크립트와 공유하지 않는 변수에서만 대상 주소를 읽으며, 시스템 설정은 절대 쓰지 않습니다. 가리킨 서비스의 플레이 정책·개인정보 정책·승인 흐름은 손대지 않은 채로 돌아옵니다. 옛 `docs/guide.md`와 `docs/igame_User_Guide.pdf`는 새 문서 옆에 남기지 않고 지웠으며, README와 두 소개 페이지, sitemap, `scripts/build-docs-pdf.sh`가 모두 새 가이드를 가리킵니다. 문서만 바뀐 릴리스이므로 제품 코드, 스키마·마이그레이션, 환경변수, API 계약과 게시된 콘텐츠는 모두 그대로입니다. `v0.7.11`이 고친 일일 플레이 시간 제한의 열린 세션 누락, `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.12`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
