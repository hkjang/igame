...
`v0.7.3`은 스키마 마이그레이션 적용 순서를 고친 패치 릴리스입니다. 마이그레이션은 파일 이름을 사전식으로 정렬한 순서로 적용되고 있었는데, 그 순서는 번호 자릿수가 모두 같을 때에만 번호가 뜻하는 순서와 일치합니다. 기존 `001_`~`009_` 옆에 처음으로 `10_`으로 시작하는 파일이 들어오면 그 파일이 두 번째로 정렬돼, 아직 만들어지지도 않은 테이블을 ALTER 하다가 운영 데이터베이스 배포 도중에 멈추게 됩니다. 이제 파일 이름이 아니라 파싱한 번호 순서로 적용하고, 번호가 없는 이름이나 이미 쓰인 번호는 파일 시스템이 고른 자리에 그대로 적용하는 대신 기동 시점에 거부합니다. 마이그레이션 파일은 `//go:embed`로 binary에 박히므로 그런 이름은 배포 환경의 차이가 아니라 저장소의 실수이며, 모든 기동이 같은 방식으로 보고해야 합니다. 스키마 자체와 동봉된 마이그레이션 목록, 환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.3`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
