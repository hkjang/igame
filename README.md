...
`v0.7.11`은 일일 플레이 시간 제한이 지금 열려 있는 세션을 세지 않던 문제를 고친 패치 릴리스입니다. `duration_ms`는 세션이 끝날 때만 기록되므로, 제한 판정이 더하던 하루 합계는 플레이어가 그 순간 열어 둔 세션을 0분으로 셌습니다. 제한은 세션을 시작할 때 확인하는데 바로 그때 열려 있는 세션이 합계에서 빠지는 그 세션이라, 이미 종료된 세션만으로 한도 아래이면 몇 시간을 앉아 있었든 새 세션이 다시 허용되었고 매일 한 세션만큼 한도를 넘겼습니다. 60분 제한에 55분씩 앉으면 110분이 됩니다. 이제 `status='active'` 행은 시작 이후 경과 시간을 함께 더합니다. 세션을 finish 할 때와 새 세션이 이전 세션을 abandoned로 닫을 때 이미 `duration_ms`에 쓰는 것과 같은 계산입니다. 끝난 세션은 기록된 duration을 그대로 쓰고, duration 없이 닫힌 세션은 시작 이후 전부를 청구당하지 않고 0으로 남습니다. 판정하는 주체가 PostgreSQL이므로 동작은 실제 데이터베이스에서 확인합니다. `internal/api/catalog_pg_test.go`의 `IGAME_TEST_DSN` 테스트가 진행 중인 세션, 서비스 하루가 시작되기 전부터 열려 있던 세션, duration 없이 닫힌 세션과 한도 아래의 여유를 각각 검사합니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.10`이 고친 잠긴 Node 의존성 감사의 릴리스 중단, `v0.7.9`가 고친 관리자 비밀번호 재설정의 세션 미폐기, `v0.7.8`이 고친 부서·팀 랭킹의 조직 공개 정책 우회, `v0.7.7`이 고친 `/assets/` 디렉터리 목록 노출, `v0.7.6`이 고친 세션 쿠키의 Secure 플래그, `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.11`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
