...
`v0.7.6`은 대문자 하나 때문에 세션 쿠키에서 Secure 플래그가 빠지던 문제를 고친 패치 릴리스입니다. URL 스킴은 대소문자를 가리지 않으므로 시스템 설정 화면은 `public_url`을 `HTTPS://games.example.com`으로도 받아들입니다 — 저장 전 검사가 읽는 `url.Parse`가 스킴을 먼저 소문자로 만들기 때문입니다. 그런데 저장된 값을 쓰는 쪽은 원문 문자열을 그대로 비교했고, 그 비교는 셋 모두 `https://`로 시작하는지 묻는 검사였습니다 — 세션 쿠키의 Secure 플래그, HSTS 헤더, 상태 화면의 TLS 표시. 그래서 TLS로 서비스되는 배포인데도 세션 쿠키가 Secure 없이 나갔고, 같은 호스트로 가는 평문 요청에 그대로 실렸습니다. 이제 `requestBaseURL`이 스킴을 소문자로 맞추고 앞뒤 공백과 끝의 `/`를 떼어 낸 정규화된 base URL을 돌려주며, 상태 화면의 TLS 표시도 같은 헬퍼를 읽습니다. 이미 대문자로 저장해 둔 배포는 아무도 눈치채고 다시 저장하지 않아도 이 릴리스만으로 고쳐집니다. 호스트 표기는 운영자가 적은 그대로 두는데, 호스트를 읽는 쪽은 이미 대소문자를 가리지 않고 비교하기 때문입니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.5`가 고친 플레이 시간 창의 판정과 편집, `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.6`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
