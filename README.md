...
`v0.7.4`는 플레이 시간 창을 설정 화면이 되돌려 보여줄 수 있는 형태로만 저장하도록 고친 패치 릴리스입니다. 창의 시작·종료 시각을 읽는 검사가 시와 분을 그대로 `strconv.Atoi`에 넘겨서 `+9:+5`나 `0009:0005`처럼 시계에 없는 표기까지 09:05로 통과시켰고, `9:00`처럼 0을 채우지 않은 값도 그대로 저장됐습니다. 그런데 그 값을 설정하는 화면은 `<input type="time">`이라 HH:MM이 아닌 값을 아예 빈칸으로 그리므로, 다음 관리자는 스위치가 켜진 채 허용 시작이 비어 있는 화면을 보고 창이 적용 중인지 알 수 없었습니다 — 그동안 서버는 그 창으로 사람들을 돌려보내고 있었습니다. 이제 시각은 부호·공백·자리 채움·두 번째 콜론 없이 한두 자리 숫자로만 읽고, 저장 검사는 화면이 되돌려 보여줄 수 있는 0 채움 형태만 받아들입니다. 실행 시점의 창 판정은 `9:00`을 계속 읽으므로 이전에 API로 직접 넣어 둔 정책은 그대로 동작하며, `24:00`은 창의 끝에서만 유효합니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.4`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
