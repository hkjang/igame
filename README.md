...
`v0.7.5`는 플레이 시간 창을 화면이 보여주는 대로 지키도록 고친 패치 릴리스입니다. 시작과 종료가 같은 창은 화면에서 길이가 0인 가장 좁은 창으로 읽히는데, 서버는 그것을 "그 요일 하루 종일 허용"으로 읽고 있었습니다. 09:00~09:00을 지정한 요일에 스위치가 켜져 있고 요일도 체크돼 있는데 아무도 막지 않는, 관리자가 요청한 것과 정반대의 상태였습니다. 이제 창은 시작부터 종료까지만 덮으므로 같은 두 시각은 그 1분만 뜻하고, 저장 검사는 그런 창을 아예 거부합니다 — 시간을 제한하지 않는 정책에는 창이 없는 것이지 길이가 0인 창이 있는 것이 아닙니다. 창 판정은 데이터베이스가 필요 없는 `playWindowsAllow`로 분리해 요일·경계·자정 넘김까지 표로 검증합니다. 같은 화면이 정책의 첫 창만 보여주고 저장해서 API로 넣어 둔 나머지 창은 보존만 되고 읽거나 고칠 수 없었고, 그중 하나라도 새 검사에 걸리면 이후 저장이 모두 막혔을 것입니다. 시스템 설정은 이제 창 목록 전체를 편집하며 창마다 삭제 버튼과 아래에 허용 시간대 추가를 두고, 창이 여럿인 정책은 그중 하나에만 들어도 플레이가 허용된다고 한국어로 설명합니다. 스키마·마이그레이션·환경변수와 게시된 콘텐츠, API 계약은 모두 그대로입니다. `v0.7.4`가 고친 플레이 시간 창의 저장 형태, `v0.7.3`이 고친 마이그레이션 적용 순서, `v0.7.2`가 고친 로그인 복귀 주소와 목록 검색, `v0.7.1`이 정리한 관리자 콘솔 동작, `v0.7.0`이 적과 타워에 부여한 개성, `v0.6.0`의 서버 재현 확정과 기존 포털 접근성·운영 도구·실행 성능 계약은 유지됩니다. 게시된 콘텐츠는 그대로이므로 새 콘텐츠 UUID로 전환하지 않고 캠페인·영웅·스킬 진행도와 기존 랭킹을 모두 보존합니다. 이전 릴리스에서 `server_received_telemetry_v1`로 검증된 기록은 그대로 남고, 새 기록은 `server_replay_v1`로 표시됩니다.
...
서비스, Docker image, web application과 `gamehub-js` SDK는 이 release에서 root `VERSION` `0.7.5`으로 정렬됩니다. RealmGuard 콘텐츠 `0.3.1`과 Defense Series 콘텐츠 `0.4.0`은 별도 수명 주기를 가지므로 서비스 버전으로 덮어쓰지 않습니다.
