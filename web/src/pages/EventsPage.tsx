...
import { Box, Button, Card, CardActions, CardContent, Chip, Container, Grid, LinearProgress, Stack, Typography } from '@mui/material';
...
export function EventsPage() {
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 4, md: 6 } }}><PageHeader icon={<CalendarMonthRounded />} title="시즌 & 이벤트" description="동료들과 함께 참여하고 특별한 배지를 획득하세요." />
    {loading && <LinearProgress sx={{ mt: 4 }} />}{error && <Box mt={4}><ErrorPanel error={error} retry={() => void reload()} /></Box>}
    {!loading && !error && events.length === 0 && <Box mt={4}><EmptyState icon={<CalendarMonthRounded />} title="진행 중인 이벤트가 없습니다" description="관리자가 새 시즌이나 이벤트를 열면 이곳에서 바로 참여할 수 있습니다. 그동안은 랭킹에서 기록을 쌓아 두세요." action={<Button component={RouterLink} to="/rankings" variant="outlined" startIcon={<EmojiEventsRounded />}>랭킹 보기</Button>} /></Box>}
