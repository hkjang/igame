...
import { DefenseReviewQueue } from '../components/DefenseReviewQueue';
...
export function ReviewsPage() {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}><Typography variant="h1" sx={{ fontSize: { xs: '2.2rem', md: '3.2rem' } }}>승인함</Typography><Typography color="text.secondary" mt={1} mb={4}>서비스 변경 요청을 검토하고 적용 여부를 결정합니다.</Typography><ReviewQueue enabled={Boolean(config.approval_enabled)} /><RealmGuardReviewQueue enabled={Boolean(config.approval_enabled)} /><DefenseReviewQueue enabled={Boolean(config.approval_enabled)} /></Container>;
}
