...
export function RankingsPage() {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
    {/* The icon sits in its own tile so it lines up with the title's cap
        height instead of floating against a two-line block. */}
    <Stack direction="row" spacing={2} alignItems="center">
      <Box aria-hidden sx={(theme) => ({ width: 56, height: 56, flexShrink: 0, borderRadius: 3, display: 'grid', placeItems: 'center', color: 'warning.main', bgcolor: alpha(theme.palette.warning.main, theme.palette.mode === 'dark' ? .18 : .12) })}><EmojiEventsRounded sx={{ fontSize: 32 }} /></Box>
      <Box><Typography variant="h1" sx={{ fontSize: { xs: '2.2rem', md: '3rem' } }}>랭킹</Typography><Typography color="text.secondary" mt={.5}>개인과 조직의 최고 기록을 확인하세요.</Typography></Box>
    </Stack>
