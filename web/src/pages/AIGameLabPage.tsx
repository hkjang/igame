...
export function AIGameLabPage() {
  ...
  // Leaving the page must not leave the upstream AI request running.
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      // Stopping keeps whatever the model already told; only a real failure is an error.
      if (controller.signal.aborted) { if (accumulated) setMessages((current) => [...current, { role: 'assistant', content: accumulated }]); }
      else setError(cause instanceof Error ? cause.message : 'AI 응답을 받지 못했습니다.');
    } finally { setDraft(''); setStreaming(false); abort.current = undefined; }
  };
  return <Container maxWidth="md" sx={{ py: { xs: 4, md: 6 } }}><Stack direction="row" spacing={1.5} alignItems="center"><Box sx={{ width: 52, height: 52, display: 'grid', placeItems: 'center', borderRadius: 2.5, bgcolor: (theme) => alpha(theme.palette.accent.ai, .16), color: 'accent.ai' }}><AutoAwesomeRounded fontSize="large" /></Box><Box><Typography variant="h1" sx={{ fontSize: { xs: '2.1rem', md: '3rem' } }}>AI Game Lab</Typography><Typography color="text.secondary">스트리밍 AI Dungeon 데모</Typography></Box></Stack><Alert severity="info" sx={{ mt: 3 }}>AI 요청은 igame 서버를 거쳐 전송되며 API Key는 브라우저에 노출되지 않습니다.</Alert><Card sx={{ mt: 3 }}><CardContent className="admin-scrollbar" aria-live="polite" sx={{ p: { xs: 2, md: 3 }, height: 'min(58vh,620px)', overflowY: 'auto' }}><Stack spacing={2}>{messages.map((message, index) => <Box key={index} sx={{ alignSelf: message.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '86%', px: 2, py: 1.5, borderRadius: 2.5, whiteSpace: 'pre-wrap', bgcolor: message.role === 'user' ? 'primary.dark' : 'action.hover', color: message.role === 'user' ? 'common.white' : 'text.primary' }}><Typography>{message.content}</Typography></Box>)}{streaming && <Box sx={{ alignSelf: 'flex-start', maxWidth: '86%', px: 2, py: 1.5, borderRadius: 2.5, bgcolor: 'action.hover', whiteSpace: 'pre-wrap' }}>{draft ? <Typography>{draft}</Typography> : <Stack direction="row" spacing={1} alignItems="center"><CircularProgress size={20} /><Typography color="text.secondary">이야기를 만드는 중…</Typography></Stack>}</Box>}</Stack></CardContent></Card>{error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}<Stack component="form" onSubmit={(event) => void submit(event)} direction={{ xs: 'column', sm: 'row' }} spacing={1.5} mt={2}><TextField value={prompt} onChange={(event) => setPrompt(event.target.value)} label="다음 행동" placeholder="예: 신호의 발신지를 추적한다" disabled={streaming} inputProps={{ maxLength: 2000 }} />{streaming ? <Button type="button" color="warning" variant="outlined" startIcon={<StopCircleRounded />} onClick={() => abort.current?.abort()}>중단</Button> : <Button type="submit" variant="contained" startIcon={<SendRounded />} disabled={!prompt.trim()}>전송</Button>}</Stack></Container>;
}
