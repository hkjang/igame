...
export function GamesPage() {
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 4, md: 6 } }}>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} mt={4} mb={3} alignItems={{ md: 'center' }}><TextField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="게임 이름이나 태그 검색" aria-label="게임 검색" sx={{ maxWidth: 520 }} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchRounded /></InputAdornment> } }} /><Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">{categories.map((item) => <Chip key={item} label={item} clickable color={category === item ? 'primary' : 'default'} variant={category === item ? 'filled' : 'outlined'} onClick={() => setCategory(item)} />)}</Stack></Stack>
