...
export const realmGuardDesignerAPI = {
  ...
  section: (section: RealmSection, versionId?: string) => api.request<{ version: RealmGuardVersionRecord; section: RealmSection; data: unknown }>(`/api/v1/admin/realmguard/drafts/${section}${versionId ? `?version_id=${encodeURIComponent(versionId)}` : ''}`),
  saveSection: (section: RealmSection, data: unknown, versionId: string, checksum: string) => api.request<{ version: RealmGuardVersionRecord; section: RealmSection; data: unknown }>(`/api/v1/admin/realmguard/drafts/${section}?version_id=${encodeURIComponent(versionId)}`, { method: 'PUT', headers: { 'If-Match': `"${checksum}"` }, body: JSON.stringify({ data }) }),
