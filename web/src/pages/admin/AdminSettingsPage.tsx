...
export function AdminSettingsPage({ section }: { section: 'oidc' | 'ai' | 'approval' | 'api_keys' | 'general' | 'tracking' }) {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 3, lg: 5 } }}><Typography variant="h1" sx={{ fontSize: { xs: '2.1rem', lg: '3rem' } }}>{title}</Typography><Typography color="text.secondary" mt={1} mb={3}>환경변수 변경 없이 관리자 페이지에서 운영 정책을 안전하게 관리합니다.</Typography>{result.loading ? <LoadingScreen /> : result.error ? <ErrorPanel error={result.error} retry={() => void result.reload()} /> : result.data && (section === 'oidc' ? <Stack spacing={3}><OIDCSettings key={revision} initial={result.data.settings.oidc ?? {}} secretStored={Boolean(result.data.secrets?.oidc?.client_secret)} /><McpOAuthSettings key={`mcp-${revision}`} initial={result.data.settings.mcp ?? {}} oidc={result.data.settings.oidc ?? {}} service={result.data.settings.service ?? {}} /></Stack> : section === 'ai' ? <AISettings key={revision} initial={result.data.settings.ai ?? {}} secretStored={Boolean(result.data.secrets?.ai?.api_key)} /> : section === 'approval' ? <ApprovalSettings key={revision} initial={result.data.settings.approval ?? {}} /> : section === 'api_keys' ? <KeyPolicySettings key={revision} initial={result.data.settings.api_keys ?? {}} /> : section === 'tracking' ? <TrackingSettings key={revision} initial={result.data.settings.tracking ?? {}} /> : <GeneralSettings key={revision} settings={result.data.settings} />)}</Container>;
}
