...
class RealmGuardBattleScene extends Phaser.Scene {
  ...
  create() {
    this.gold = calculateStartingGold(
      this.options.stage,
  ...
  command(command: RealmCommand) {
    ...
    switch (command.type) {
      ...
      case "speed":
        ...
        break;
      case "build":
        this.buildTower(command.tower, command.profile);
        break;
      case "upgrade":
        this.upgradeTower(command.branch);
        break;
      case "sell":
        this.sellTower();
        break;
      case "targeting":
        this.changeTargeting(command.mode);
        break;
      case "skill":
      ...
      case "move-hero":
        ...
        break;
      case "adjust-economy":
        this.gold = applyResourceDelta(this.gold, command.resourceDelta);
        if (command.resourceDelta >= 0)
          this.earnedGold += Math.round(command.resourceDelta);
        else this.spentGold += Math.abs(Math.round(command.resourceDelta));
        this.lives = Math.max(
          0,
          this.lives + Math.round(command.healthDelta ?? 0),
        );
        this.emitHUD(true);
        if (this.lives <= 0) void this.endBattle(false);
        break;
      case "force-defeat":
        void this.endBattle(false);
        break;
    }
  }
  ...
  private createTowerSpots() {
    for (const spot of this.options.stage.towerSpots) {
      ...
      ring.fillStyle(0x0a1623, 0.78).fillCircle(0, 0, 29);
      ring.lineStyle(3, 0x8bd8c5, 0.72).strokeCircle(0, 0, 27);
      ring.lineStyle(1, 0xffffff, 0.16).strokeCircle(0, 0, 20);
      const plus = this.add
        .text(0, -2, "+", {
          fontFamily: "system-ui",
          fontSize: "30px",
  ...
  private startWave(requestedEarly: boolean) {
    ...
    if (stage.mode === "campaign" && this.waveIndex >= stage.waves.length)
      return;
    ...
    this.options.onTelemetry("realmguard.wave.start", {
      stage_id: stage.id,
  ...
  private spawnEnemy(order: SpawnOrder) {
    const definition =
      this.options.config.enemies.find((item) => item.id === order.enemy) ??
      this.options.config.enemies[0];
    const difficulty =
      this.options.config.balance.difficulties[this.options.difficulty];
    const endlessScale =
      1 +
      Math.floor(
        this.waveIndex / Math.max(1, this.options.stage.waves.length),
      ) *
        this.options.config.balance.endlessRamp;
    ...
    const body = this.add.graphics();
    const flying =
      definition.traits.includes("flying") || modifiers.has("flying");
    ...
    const object = this.add
      .container(
        start.x,
        start.y - (flying ? 20 : 0),
        label ? [body, health, label] : [body, health],
      )
      .setDepth(flying ? 8 : 6);
    ...
    this.spawned += 1;
    this.spawnedByEnemy[definition.id] =
      (this.spawnedByEnemy[definition.id] ?? 0) + 1;
  ...
  private updateEnemy(enemy: EnemyUnit, time: number, delta: number) {
    ...
    if (
      ...
      const barracks = [...this.towers.values()].find(
        (tower) =>
          this.isBlockingTower(tower.definition) &&
          ...
          tower.soldiers?.some(
            (soldier) =>
              ...
              enemy.definition.radius + 22,
          ) &&
          (tower.blockedEnemies.has(enemy.id) ||
            tower.blockedEnemies.size < (tower.soldiers?.length ?? 0)),
      );
      if (barracks) {
        const firstBlock = !barracks.blockedEnemies.has(enemy.id);
        barracks.blockedEnemies.add(enemy.id);
        if (firstBlock)
          this.options.onTelemetry("realmguard.barracks.block", {
            enemy: enemy.definition.id,
            tower: barracks.definition.id,
          });
        if (time - enemy.lastAttack >= 850) {
          enemy.lastAttack = time;
          const soldier =
            barracks.soldiers?.[
              enemy.id % Math.max(1, barracks.soldiers.length)
            ];
          if (soldier)
            this.pulse(soldier.x, soldier.y, barracks.definition.color, 24);
    ...
    if (
      ...
      Phaser.Math.Distance.Between(
        ...
        enemy.definition.radius + 28
    ) {
      if (time - enemy.lastAttack >= 850) {
        enemy.lastAttack = time;
        this.damageHero(
          Math.max(8, enemy.definition.lifeDamage * 11 + enemy.maxHp * 0.008),
        );
        if (this.hasTrait(enemy, "siege")) this.siegeDisrupt(enemy);
      }
      return;
    ...
    const speed =
      enemy.speed *
      movementMultiplier(
        mergedTraits(enemy.definition, enemy.modifiers),
  ...
  private hasTrait(enemy: EnemyUnit, trait: string) {
    return (
      enemy.definition.traits.includes(
        trait as EnemyArchetype["traits"][number],
      ) || enemy.modifiers.has(trait)
    );
  }
  ...
  private enemyEscaped(enemy: EnemyUnit) {
    ...
    this.escaped += 1;
    this.escapedByEnemy[enemy.definition.id] =
      (this.escapedByEnemy[enemy.definition.id] ?? 0) + 1;
  ...
  private updateTower(tower: TowerUnit, time: number) {
    if (tower.disabledUntil <= time && tower.object.alpha < 1) {
      ...
      tower.soldiers?.forEach(
        (soldier) => soldier.active && soldier.setAlpha(1),
      );
    }
    ...
    if (
      tower.disabledUntil > time ||
      !simulationCooldownReady(time, tower.lastShot, stats.fireRate * 1000)
    )
      return;
    const targets = [...this.enemies.values()].filter((enemy) => {
      ...
      return (
        ...
        (!this.isBlockingTower(tower.definition) ||
          tower.branch === tower.definition.branches[1]?.id ||
          !this.hasTrait(enemy, "flying"))
      );
    });
  ...
  private fireProjectile(
    tower: TowerUnit,
    target: EnemyUnit,
    stats: ReturnType<typeof calculateTowerStats>,
  ) {
    const targetX = target.object.x;
    const targetY = target.object.y;
    const profileMultiplier =
      tower.definition.profiles?.find((profile) => profile.id === tower.profile)
        ?.damageMultiplier ?? 1;
    this.damageEnemy(
      target,
      stats.damage *
        profileMultiplier *
        towerEffectivenessMultiplier(tower.definition, target.definition),
        ...
        if (
          ...
          this.damageEnemy(
            ...
            stats.damage *
              profileMultiplier *
              0.48 *
              towerEffectivenessMultiplier(tower.definition, enemy.definition),
            tower.definition.damageType,
            stats.slow,
          );
    if (stats.pierce > 0) {
    ...
    this.pulse(
      ...
      Math.max(20, stats.splash),
    );
    if (this.isBlockingTower(tower.definition)) {
      const soldier =
        tower.soldiers?.[
          Math.floor(tower.lastShot / 50) % Math.max(1, tower.soldiers.length)
        ];
      if (soldier)
        this.tweens.add({
          targets: soldier,
          x: Phaser.Math.Linear(soldier.x, targetX, 0.42),
          y: Phaser.Math.Linear(soldier.y, targetY, 0.42),
          duration: 120,
          yoyo: true,
        });
      return;
    ...
    const projectile = this.add
      .circle(
        tower.object.x,
        tower.object.y,
        tower.definition.damageType === "siege" ? 7 : 4,
        tower.definition.color,
      )
      .setDepth(12);
    this.tweens.add({
  ...
  private damageEnemy(
    ...
    if (!enemy.alive) return;
    if (
      this.hasTrait(enemy, "phasing") &&
      type !== "skill" &&
      ++this.hitSequence % 4 === 0
    ) {
      this.pulse(enemy.object.x, enemy.object.y, 0xa78bfa, 34);
      return;
    }
    ...
    if (
      slow &&
      !this.hasTrait(enemy, "boss") &&
      !this.hasTrait(enemy, "immune_stun")
    ) {
  ...
  private killEnemy(enemy: EnemyUnit) {
    ...
    this.kills += 1;
    this.defeatedByEnemy[enemy.definition.id] =
      (this.defeatedByEnemy[enemy.definition.id] ?? 0) + 1;
    this.heroGainXp(1);
  ...
  private spawnSplit(parent: EnemyUnit) {
    const definition = this.options.config.enemies.find(
      (item) => item.id === "mireling",
    )!;
    const body = this.add
      .graphics()
    ...
    const object = this.add
      .container(
        parent.object.x + Phaser.Math.Between(-8, 8),
        parent.object.y + Phaser.Math.Between(-8, 8),
        [body, health],
      )
      .setDepth(6);
    const enemy: EnemyUnit = {
  ...
  private removeEnemy(enemy: EnemyUnit) {
    ...
    for (const tower of this.towers.values())
      tower.blockedEnemies.delete(enemy.id);
  ...
  private drawEnemyHealth(enemy: EnemyUnit) {
    ...
    enemy.health
      ...
      .fillRect(-17, -enemy.definition.radius - 11, 34, 5);
    enemy.health
      .fillStyle(enemy.hp / enemy.maxHp > 0.35 ? 0x65e392 : 0xff6f72)
      .fillRect(
        -16,
        -enemy.definition.radius - 10,
        32 * Math.max(0, enemy.hp / enemy.maxHp),
        3,
      );
  }
  ...
  private updateHero(time: number, delta: number) {
    ...
    if (hero.deadUntil > 0) {
      ...
      const start =
        this.options.stage.path[
          Math.min(2, this.options.stage.path.length - 1)
        ];
      ...
      this.options.onTelemetry("realmguard.hero.respawn", {
        hero_id: hero.definition.id,
        level: hero.level,
      });
    }
    const distance = Phaser.Math.Distance.Between(
    ...
    if (distance > 4) {
      ...
      hero.object.x += Math.cos(angle) * step;
      hero.object.y += Math.sin(angle) * step;
    }
    if (
      !simulationCooldownReady(
        time,
        hero.lastShot,
        Math.max(420, 920 - hero.level * 35),
      )
    )
      return;
    ...
    if (hero.attackCount % 5 === 0) {
      damage *= 1.75;
      this.options.onTelemetry("realmguard.hero.skill", {
        hero_id: hero.definition.id,
        skill: hero.definition.skill1,
      });
    }
    this.damageEnemy(
      target,
      damage,
      "hero",
      hero.definition.id === "nyra" ? 0.25 : 0,
    );
      ...
      for (const enemy of [...this.enemies.values()])
        if (
          ...
          this.damageEnemy(
            ...
            damage * 0.55,
            "hero",
            hero.definition.id === "brann" ? 0.7 : 0.2,
          );
      this.options.onTelemetry("realmguard.hero.skill", {
        hero_id: hero.definition.id,
        skill: hero.definition.skill2,
      });
    }
    if (hero.attackCount % 25 === 0) {
      for (const enemy of [...this.enemies.values()])
        this.damageEnemy(
          ...
          damage * 0.72,
          "hero",
          hero.definition.id === "nyra" ? 0.6 : 0.15,
        );
      this.cameras.main.flash(180, 255, 224, 125, false);
      this.options.onTelemetry("realmguard.hero.ultimate", {
        hero_id: hero.definition.id,
  ...
  private damageHero(amount: number) {
    ...
    if (hero.hp > 0) return;
    hero.deadUntil =
      this.simulationTime + hero.definition.respawnSeconds * 1000;
    hero.object.setVisible(false);
    this.options.onTelemetry("realmguard.hero.defeated", {
      hero_id: hero.definition.id,
      respawn_seconds: hero.definition.respawnSeconds,
    });
    this.emitHUD(true);
  }
  ...
  private siegeDisrupt(enemy: EnemyUnit) {
    ...
    tower.object.setAlpha(0.35);
    tower.soldiers?.forEach((soldier) => soldier.setAlpha(0.35));
    this.options.onTelemetry("realmguard.enemy.siege_disrupt", {
      enemy: enemy.definition.id,
      tower: tower.definition.id,
    });
  }
  ...
  private handleBossPhases(enemy: EnemyUnit) {
    ...
    if (ratio <= 0.66 && !enemy.phases.has("phase-2")) {
      ...
      if (enemy.definition.id === "hollow_king") {
        const tower = [...this.towers.values()][
          this.waveIndex % Math.max(1, this.towers.size)
        ];
        if (tower) {
          tower.disabledUntil = this.simulationTime + 5000;
          tower.object.setAlpha(0.3);
          tower.soldiers?.forEach((soldier) => soldier.setAlpha(0.3));
        }
      } else this.spawnBossMinions(enemy, 3, "glintfox");
      this.pulse(enemy.object.x, enemy.object.y, enemy.definition.color, 120);
      this.options.onTelemetry("realmguard.boss.phase", {
        boss: enemy.definition.id,
        phase: 2,
      });
    }
    if (ratio <= 0.33 && !enemy.phases.has("phase-3")) {
      ...
      if (enemy.definition.id === "hollow_king")
        this.spawnBossMinions(enemy, 5, "veilrunner");
      else {
        enemy.speed *= 1.65;
  ...
  private spawnBossMinions(boss: EnemyUnit, count: number, enemyId: string) {
    for (let index = 0; index < count; index += 1) {
      ...
      minion.object.setPosition(
        boss.object.x + Phaser.Math.Between(-18, 18),
        boss.object.y + Phaser.Math.Between(-18, 18),
      );
    }
  }
  ...
  private updateStageGimmick(time: number) {
    ...
    if (this.options.stage.gimmick === "ember_vents") {
      ...
      for (const enemy of [...this.enemies.values()])
        if (
          ...
          this.damageEnemy(enemy, 58, "skill");
    ...
    this.options.onTelemetry("realmguard.stage.gimmick", {
      stage_id: this.options.stage.id,
      gimmick: this.options.stage.gimmick,
    });
  }
  ...
  private heroGainXp(amount: number) {
    ...
    this.hero.xp += amount;
    const required =
      this.options.config.balance.heroLevelXp[this.hero.level] ??
  ...
  private moveHero(point: Phaser.Math.Vector2) {
    if (!this.hero) return;
    this.hero.target = new Phaser.Math.Vector2(
      Phaser.Math.Clamp(point.x, 35, WIDTH - 35),
      Phaser.Math.Clamp(point.y, 70, HEIGHT - 35),
    );
    this.options.onTelemetry("realmguard.hero.move", {
      x: Math.round(point.x),
      y: Math.round(point.y),
    });
  }

  private buildTower(towerId: string, profile?: string) {
    if (!this.selectedSpot || this.towers.has(this.selectedSpot)) return;
    const definition = this.options.config.towers.find(
      (tower) => tower.id === towerId,
    );
    const spot = this.options.stage.towerSpots.find(
      (item) => item.id === this.selectedSpot,
    );
    if (
      !definition ||
      !spot ||
      this.gold < definition.cost ||
      (profile && !definition.profiles?.some((item) => item.id === profile))
    )
      return;
    this.gold -= definition.cost;
    ...
    const soldiers = this.isBlockingTower(definition)
      ? this.createBarracksSoldiers(spot.x, spot.y)
      : undefined;
    this.towers.set(spot.id, {
      spotId: spot.id,
      definition,
      object,
      level: 1,
    ...
    this.options.onTelemetry("realmguard.tower.build", {
      tower: towerId,
      spot: spot.id,
      profile_id: profile,
    });
  ...
  private createBarracksSoldiers(x: number, y: number) {
    ...
    return [-18, 18].map((offset) => {
      const shield = this.add
        ...
        .strokeCircle(0, 0, 9);
      return this.add
        .container(nearest.x + offset, nearest.y + (offset > 0 ? 16 : -16), [
          shield,
        ])
        .setDepth(8);
    });
  ...
  private upgradeTower(branch?: string) {
    ...
    const cost =
      this.options.config.balance.towerUpgradeCost[tower.level] ?? 100;
    ...
    if (
      tower.level === 3 &&
      branch &&
      tower.definition.branches.some((item) => item.id === branch)
    )
      tower.branch = branch;
    tower.object.removeAll(true);
    tower.object.add(this.drawTowerShape(tower.definition, tower.level));
    this.pulse(tower.object.x, tower.object.y, tower.definition.color, 52);
    this.options.onTelemetry("realmguard.tower.upgrade", {
      tower: tower.definition.id,
      spot: tower.spotId,
      level: tower.level,
      branch: tower.branch,
  ...
  private sellTower() {
    ...
    if (!tower) return;
    const invested =
      tower.definition.cost +
      this.options.config.balance.towerUpgradeCost
        .slice(1, tower.level)
        .reduce((sum, value) => sum + value, 0);
    const refund = Math.round(
      invested * this.options.config.balance.sellRefundRate,
    );
    this.gold += refund;
    this.soldGold += refund;
    tower.soldiers?.forEach((soldier) => soldier.destroy(true));
    tower.object.destroy(true);
    this.towers.delete(this.selectedSpot);
    const spot = this.options.stage.towerSpots.find(
      (item) => item.id === this.selectedSpot,
    )!;
    const ring = this.add
  ...
  private changeTargeting(mode: TargetingMode) {
    ...
    this.options.onTelemetry("realmguard.tower.targeting", {
      tower: tower.definition.id,
      mode,
    });
    this.emitHUD(true);
  ...
  private armSkill(skillId: string) {
    const skill = this.options.config.skills.find(
      (item) => item.id === skillId,
    );
    if (!skill || (this.skillReady.get(skillId) ?? 0) > this.simulationTime)
      return;
    ...
    } else if (skillId === "meteor" || skillId === "reinforcement")
  ...
  private castMeteor(point: Phaser.Math.Vector2) {
    for (const enemy of this.enemies.values())
      if (
        ...
        this.damageEnemy(enemy, 245, "skill");
    const impact = this.add
      .circle(point.x, point.y, 8, 0xff774e, 0.95)
      .setDepth(14);
    this.tweens.add({
      targets: impact,
      radius: 125,
    ...
    this.options.onTelemetry("realmguard.skill.cast", {
      skill: "meteor",
      x: Math.round(point.x),
      y: Math.round(point.y),
    });
  ...
  private castReinforcement(point: Phaser.Math.Vector2) {
    ...
    const object = this.add
      .container(point.x, point.y, [ward, blades])
      .setDepth(8);
  ...
  private updateReinforcements(time: number) {
    ...
    this.reinforcements = this.reinforcements.filter(
      (unit) => unit.object.active && time < unit.expiresAt,
    );
  }

  private completeWave() {
    const wave =
      this.options.stage.waves[
        this.waveIndex % this.options.stage.waves.length
      ];
    ...
    if (
      canResolveCampaignVictory(
        this.completed,
        this.options.stage.mode,
        this.waveIndex,
        this.options.stage.waves.length,
      )
    )
      void this.endBattle(true);
    else {
      this.gameStatus = "ready";
      this.nextWaveAt = this.simulationTime + 10_000;
      this.emitHUD(true);
    }
  }
  ...
  private async endBattle(victory: boolean) {
    if (this.completed) return;
    this.completed = true;
    this.gameStatus = victory ? "victory" : "defeat";
    const duration = Math.round(this.activeWallMs);
    const remainingGold = visibleResource(this.gold);
  ...
  private emitHUD(force = false) {
    ...
    const selected = this.selectedSpot
      ? this.towers.get(this.selectedSpot)
      : undefined;
    const cooldowns: Record<string, number> = {};
    for (const skill of this.options.config.skills)
      cooldowns[skill.id] = Math.max(
        0,
        Math.ceil(
          ((this.skillReady.get(skill.id) ?? 0) - this.simulationTime) / 1000,
        ),
      );
    this.options.onHUD({
      ...
      totalWaves:
        this.options.stage.mode === "endless"
          ? 0
          : this.options.stage.waves.length,
      kills: this.kills,
      ...
      heroRespawn: this.hero?.deadUntil
        ? Math.max(
            0,
            Math.ceil((this.hero.deadUntil - this.simulationTime) / 1000),
          )
        : 0,
