...
| 구성 요소 | 형태 | 역할 | 비고 |
| --- | --- | --- | --- |
| `igame` | 컨테이너 1개 (`igame:v0.7.16`) | API·포털·게임 자산·MCP 엔드포인트를 모두 제공 | 최종 runtime은 `scratch` 기반. shell·패키지 매니저 없음 |
...
```bash
sha256sum igame-v0.7.16.tar.gz
gzip -t igame-v0.7.16.tar.gz
gzip -dc igame-v0.7.16.tar.gz | docker load
docker image inspect igame:v0.7.16 --format '{{json .RepoTags}}'
```
...
`/readyz`가 `{"service":"igame","status":"ok","version":"0.7.16"}`을 돌려주면 데이터베이스까지
붙은 것입니다.
...
`/admin/mail`에서 사내 SMTP 릴레이로 이벤트 알림을 보내도록 설정합니다. **기본값은 꺼짐**이라
새로 설치한 곳에서는 아무것도 달라지지 않습니다. 메일은 요청과 별도로 **배경에서** 나가므로
릴레이가 느리거나 죽어 있어도 승인 한 건이 기다리지 않습니다 — 발송 실패는 그 요청의 실패가
아니라 발송 기록의 실패 한 줄입니다.

**설정 표.** 키 이름은 사내 메일 표준(참조 구현 kanpic)과 같습니다. 저장 단위는 `mail` 설정
하나이고 그 안의 필드가 아래 키입니다.

| 키 | 기본값 | 뜻 |
| --- | --- | --- |
| `mail.enabled` | `false` | 꺼짐이 기본. 관리자가 켭니다 |
| `mail.smtp_host` | — | 사내 릴레이 주소. 켤 때 필수 |
| `mail.smtp_port` | `25` | 사내 릴레이는 대개 25. 465면 `security`가 자동으로 `tls`가 됩니다 |
| `mail.security` | `auto` | `auto` · `none` · `starttls` · `tls`. `auto`는 서버가 STARTTLS를 알리면 쓰고 아니면 평문 |
| `mail.skip_tls_verify` | `false` | 사내 인증서가 사설일 때만 |
| `mail.username` · `mail.password` | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택**. 사용자 이름을 비우면 비밀번호도 저장하지 않습니다 |
| `mail.from_address` · `mail.from_name` | — · `igame` | 보내는 사람. 주소는 켤 때 필수 |
| `mail.base_url` | — | 메일 속 링크가 가리킬 이 서비스의 주소. 비우면 링크를 넣지 않습니다 |
| `mail.timeout_seconds` | `10` | 연결부터 전송까지의 제한 시간(1–300) |
| `mail.notify_approval_requested` | `true` | 승인 요청 도착 |
| `mail.notify_approval_decided` | `true` | 승인 결과 |
| `mail.notify_ranking_moderated` | `true` | 내 점수 조정 |

**비밀번호는 되읽히지 않습니다.** 설정 API(`GET /api/v1/admin/settings`, `GET /api/v1/admin/settings/mail`)는
비밀번호를 돌려주지 않고 `password_configured`(또는 `secrets.mail.password`)로 "설정됨"만
알립니다. 화면에서 비밀번호 칸을 비운 채 저장하면 저장된 값이 유지되고, 바꿀 때만 새 값을
넣습니다. 값은 `IGAME_ENCRYPTION_KEY`로 암호화되어 저장되며, 감사 로그에는 `password: replaced`로만
남습니다. 저장된 설정을 잠시 읽지 못한 순간의 저장은 `503 mail_setting_unavailable`로 거부됩니다 —
빈 비밀번호를 덮어쓰지 않기 위해서입니다.

**보내는 이벤트.** kanpic의 목록을 옮기지 않고, 이 서비스에서 **사람이 실제로 기다리는 일** 세
가지만 골랐습니다. 기준은 하나입니다 — 이 메일이 오지 않으면 누군가 손해를 보거나 화면을 계속
새로고침한다.

| 이벤트 | 언제 | 받는 사람 |
| --- | --- | --- |
| 승인 요청 도착 | 게임 등록·수정 요청이 대기열에 들어올 때, RealmGuard·Defense 콘텐츠의 게시 승인이 요청될 때 | 관리자 전원, 요청자(콘텐츠는 작성자) 팀의 팀장. 승인 정책이 팀장 필수가 아니면 운영자도 |
| 승인 결과 | 검토자가 승인하거나 반려할 때 | 요청한 사람(콘텐츠는 작성자). 반려 사유가 함께 갑니다 |
| 내 점수 조정 | 운영자가 점수를 제외·검토 표시·복구할 때 | 그 점수의 주인. 같은 상태를 다시 저장하면 보내지 않습니다 |

시끄러우면 사람들은 규칙을 만들어 통째로 버리므로, **자기가 한 일은 자기에게 보내지 않고**(관리자
겸 팀장이 자기 요청을 올려도 받지 않음), 한 사람이 두 자격으로 받게 되면 주소 기준으로 한 통만
보냅니다. 비활성 계정, 이메일이 비어 있는 계정에는 보내지 않으며, 본인이 `PUT /api/v1/me/preferences`에
`{"mail_opt_out": true}`를 두면 그 사람에게는 어떤 알림도 가지 않습니다. 사용자 명부는 따로 두지
않습니다 — 계정 id를 이메일로 바꾸는 조회 하나만 사용자 표에서 빌려 씁니다.

**시험 발송.** 릴레이 설정은 한 번에 맞는 일이 드뭅니다. 설정을 **저장한 뒤** 같은 화면의 **시험
발송**에 받는 주소(기본은 로그인한 관리자 본인의 이메일)를 넣고 누르면 저장된 설정으로 실제 한
통을 보내고 릴레이의 대답을 그 자리에 보여 줍니다. `connection refused`면 주소·포트·방화벽을,
`535`면 인증을, `STARTTLS`가 언급되면 보안 방식을 보면 됩니다. 꺼져 있으면 `409 mail_disabled`로
거부합니다. 시험 발송은 감사 로그에 `mail.test`로 남습니다.

**발송 기록.** 같은 화면 아래에 시도마다 한 줄씩 — 시각, 이벤트, 받는 사람, 제목, 상태(보냄·실패·
대기), 시도 횟수, 오류 — 남습니다. 실패만 남기면 "안 왔다"는 문의에 답할 수 없으므로 성공도
남깁니다. **본문은 기록하지 않습니다.** 기록은 `mail_deliveries` 테이블에 있고 백업에 포함됩니다.
실패한 발송은 2초 뒤 한 번 재시도하며, 그 뒤에도 실패하면 기록에 `실패 (2회)`로 남습니다.

**폐쇄망 권장 조합.** 릴레이 주소로 사내 메일 서비스 `postra`를 가리키면 사내 알림이 밖으로
나가지 않습니다. 표준은 어떤 릴레이든 받지만 폐쇄망 설치에서는 이 조합을 권합니다.
...
| 알림 메일이 안 옴 | `/admin/mail`의 발송 기록과 시험 발송 결과, 로그의 `notification mail failed` 경고 | 기록이 아예 없으면 꺼져 있거나 받는 사람에게 이메일이 없는 것이고, `실패`면 오류 열에 릴레이의 대답이 있습니다. 승인 한 건은 릴레이와 무관하게 처리됩니다 |
