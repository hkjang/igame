...
| 구성 요소 | 형태 | 역할 | 비고 |
| --- | --- | --- | --- |
| `igame` | 컨테이너 1개 (`igame:v0.7.17`) | API·포털·게임 자산·MCP 엔드포인트를 모두 제공 | 최종 runtime은 `scratch` 기반. shell·패키지 매니저 없음 |
...
```bash
sha256sum igame-v0.7.17.tar.gz
gzip -t igame-v0.7.17.tar.gz
gzip -dc igame-v0.7.17.tar.gz | docker load
docker image inspect igame:v0.7.17 --format '{{json .RepoTags}}'
```
...
`/readyz`가 `{"service":"igame","status":"ok","version":"0.7.17"}`을 돌려주면 데이터베이스까지
붙은 것입니다.
