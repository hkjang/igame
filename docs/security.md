...
관리자가 사용자 관리 화면에서 다른 계정의 비밀번호를 재설정하면 그 계정의 session도 함께 폐기됩니다. 탈취된 계정을 되찾는 수단이 비밀번호 재설정이므로 옛 비밀번호로 열린 session이 남아 있으면 안 됩니다. 폐기된 session 수는 `user.update` 감사 항목의 `sessions_revoked`에 기록됩니다. 자기 자신의 비밀번호를 이 화면에서 재설정할 때는 요청을 보낸 session만 유지됩니다. 계정을 `disabled`로 바꾸는 것도 즉시 효력이 있습니다. 인증 질의가 매 요청마다 `status`와 역할을 다시 읽기 때문입니다. API 키는 별도 수명 주기이므로 비밀번호 재설정으로 폐기되지 않습니다. 필요하면 키를 직접 폐기해야 합니다.
...
RealmGuard의 `server_replay_v1` attestation은 서버가 게시된 콘텐츠와 플레이어의 입력 원장만으로 전투를 처음부터 다시 실행해 결과를 확정했다는 뜻입니다. 적 이동, 타겟팅, 피해 계산, 보스 단계, 분열과 지상 저지까지 모두 서버 코드가 수행하므로 수정된 client는 자신의 입력을 바꿀 수는 있어도 그 입력의 결과를 바꿀 수 없습니다. 전투 규칙은 브라우저와 Go에 각각 구현된 결정론적 kernel 한 쌍이며, 고정 50ms step·무난수·사칙연산과 제곱근만으로 작성해 같은 배정밀도 결과를 냅니다. 두 구현은 저장소에 커밋된 replay vector와 콘텐츠 투영 digest fixture로 묶여 있고, 실행 시에도 브라우저가 보낸 투영 digest가 서버 투영과 다르면 결과를 매기지 않고 거부합니다.

Defense Series의 `server_received_telemetry_v1` attestation은 인증된 session token으로 브라우저가 자가 보고한 event를 서버가 수신해 순서·시간·누적 원장 일관성을 검증했다는 보장입니다. 서버가 전투를 독립적으로 재실행한 replay 증명은 아니므로 운영 이상 탐지를 병행합니다. RealmGuard에서도 이 telemetry 검증은 그 전투가 실제 해당 session에서 진행됐음을 확인하는 두 번째 방어선으로 유지됩니다. 브라우저의 로컬 score, 결과 payload의 client `proof`와 호환용 `events` 묶음은 공식 증거로 신뢰하거나 저장하지 않으며, 서버가 attestation digest 기반 암호화 receipt를 생성합니다.
