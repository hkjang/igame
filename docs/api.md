...
| GET | `/api/v1/admin/audit` | admin session 또는 admin 역할 + `admin:*` 키, 감사 조회. `limit`(기본 50, 최대 200)·`offset`·`q`를 받으며 응답에 필터 적용 후 전체 건수 `total`을 함께 반환합니다. `q`는 수행자 아이디·작업·대상 유형·대상 ID·IP를 부분 일치로 검색합니다. `format=csv`를 붙이면 같은 `q` 조건의 **전체 기록**을 UTF-8 BOM CSV로 스트리밍하며(`limit`/`offset` 무시), 내보내기 자체도 `audit.export`로 감사에 남습니다. 스트리밍 도중 데이터베이스 읽기가 실패하면 마지막 행으로 `export.truncated`(앞선 행 수·`request_id`)를 쓴 뒤 마지막 chunk 없이 연결을 끊어 완전한 파일로 보이지 않게 하고, `audit.export` 항목에 `truncated: true`를 남깁니다 |

OIDC client secret과 AI API key는 write-only입니다. 설정 조회는 원문 대신 빈 문자열을 돌려주고, 값이 저장되어 있는지는 `GET /api/v1/admin/settings` 응답의 `secrets` map(`{"oidc":{"client_secret":true}}` 형태)이 설정 값 **바깥**에서 알려줍니다. 단건 조회(`GET /api/v1/admin/settings/oidc`, `.../ai`)는 `client_secret_configured`·`api_key_configured`를 같은 방식으로 값 옆에 둡니다. 설정 값 안에는 그 설정이 실제로 가진 field만 들어가므로, 조회 응답을 그대로 저장 요청 본문으로 되돌려 보낼 수 있습니다. `oidc`와 `ai` 저장은 모르는 field를 거부하므로(claim 이름 오타가 조용히 무시되지 않도록), 이 왕복 규칙이 곧 계약입니다. `scripts/smoke-settings.sh`가 릴리스마다 이를 검사합니다. 저장 요청이 secret을 비워 보내면 이미 저장된 값을 그대로 유지하는데, 그 값을 읽지 못하면 빈 값으로 덮어쓰지 않고 `503 oidc_setting_unavailable`·`503 ai_setting_unavailable`로 거부합니다(해당 key가 아직 없는 최초 저장은 그대로 진행됩니다).

**Silent SSO.** `GET /api/v1/auth/oidc/login`은 `return_to`(이 서비스 안의 경로만, `/`로 시작하고 `//`로 시작하지 않는 값)와 `prompt=none`을 받습니다. `prompt=none`은 OIDC 설정의 `auto_login`이 켜져 있을 때만 제공자에게 전달되며, 꺼져 있으면 무시하고 평범한 로그인으로 진행합니다 — 주소에 붙이는 것만으로 흐름을 바꿀 수 없습니다. 조용한 시도가 세션이 없어 거절되면(`error=login_required` 등) callback은 401 대신 `302 /login?sso=none[&return_to=...]`로 보내고, 그 flow의 state는 소비됩니다. 조용하지 않은 flow의 제공자 오류는 종전처럼 `401 oidc_error`입니다. `GET /api/v1/public/config`는 `oidc_auto_login`(OIDC가 켜져 있고 `auto_login`도 켜진 경우에만 `true`)을 함께 내려 브라우저가 시도 여부를 정합니다.
...
{"metadata":{"client":"gamehub-js","client_version":"0.7.16","defense_content_version_id":"9ea33ec1-39a7-4e65-ad57-ae11a6b2790f"}}
```
...
{"metadata":{"client":"gamehub-js","client_version":"0.7.16","realmguard_version_id":"9ea33ec1-39a7-4e65-ad57-ae11a6b2790f"}}
```
